...
- **Tracker color owns the accent** (see [decision 0004](docs/decisions/0004-contrast-corrected-tint.md), superseded by [decision 0005](docs/decisions/0005-remove-colorful-controls-flag.md)) — once the intent screen resolves a prompt, the tracker's palette color takes over every downstream control (CTAs, chips, steppers, nav chrome, reveal chart), not just the card fill. Text-bearing controls use `Color.readableControlTint(in:ratio:)` (`Extensions/Color+Extensions.swift`) to pull the color into a WCAG luminance band per color scheme; `Metric.displayColor(in:)` reuses it so dashboard/charts/editors/detail share the same shade. **Card fills stay raw.** `colorScheme` threads into `MetricViewFactory`/`MetricInputFactory` because the correction is scheme-dependent.
- **Reveal vs. persistence:** the reveal card is seeded with `Metric.fakeData` so its chart looks alive, but the metric that lands on the dashboard (`persistMetric`) starts **empty**. Every kind is seeded this way, date and binary included — [decision 0010](docs/decisions/0010-fake-data-on-every-reveal-card.md) reversed the earlier "today only" exception ([0001](docs/decisions/0001-date-reveal-card.md)) after a lone real cell tested as *more* confusing than a full calendar. `doneMetric()`'s switch now only tunes how many days to seed per chart type.
